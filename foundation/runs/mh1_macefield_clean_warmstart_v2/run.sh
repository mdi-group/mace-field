...
CHECKPOINT="${RUN_ROOT}/checkpoints/mh1_macefield_clean_warmstart_v2_run-123_epoch-35.pt"
...
# This retry has its own name and output root.  It loads only the copied epoch-35
# checkpoint and cannot overwrite either the paused production run or retry 1.
exec torchrun \
  --standalone \
  --nproc_per_node=4 \
  ...
  --distributed \
  --launcher=torchrun \
  --name=mh1_macefield_clean_warmstart_v2 \
  --model=MACEField \
