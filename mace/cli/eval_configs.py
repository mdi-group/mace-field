...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--compute_becs",
        help="compute Born effective charges for MACEField",
        action="store_true",
        default=False,
    )
    parser.add_argument(
...
def get_model_output(
    ...
    if compute_polarization:
        forward_args["compute_polarization"] = True
        forward_args["training"] = True
    if compute_becs:
        forward_args["compute_becs"] = True
        forward_args["training"] = True
    if compute_polarizability:
        forward_args["compute_polarizability"] = True
        forward_args["training"] = True
...
def run(args: argparse.Namespace) -> None:
    ...
    if not hasattr(args, "compute_bec"):
        args.compute_bec = False
    ...
    for batch in data_loader:
        ...
        if args.compute_polarization:
            polarization_list.append(torch_tools.to_numpy(output["polarization"]))
    ...
    if args.compute_polarizability:
        polarizability_list = [
            polarizability
            for sublist in polarizability_list
            for polarizability in np.atleast_1d(sublist)
        ]
    if len(qs_list) > 0:
    ...
    for i, (atoms, energy, forces) in enumerate(zip(atoms_list, energies, forces_list)):
        ...
        if args.compute_polarization:
            atoms.info[args.info_prefix + "polarization"] = polarization_list[i]
        if args.compute_becs:
            atoms.arrays[args.info_prefix + "becs"] = np.asarray(
                becs_field_list[i]
            ).reshape(len(atoms), 9)
        if args.compute_polarizability:
            atoms.info[args.info_prefix + "polarizability"] = np.asarray(
                polarizability_list[i]
            ).reshape(9)
        if len(qs_list) > 0:
