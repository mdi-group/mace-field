    ...
    def forward(  # pylint: disable=arguments-differ
        ...
        for i, (interaction, product) in enumerate(
            ...
            if i < self.num_interactions - 1:
                # TorchScript does not support indexing a ModuleList with a
                # runtime integer. Iterating over the paired field modules
                # preserves the layer mapping while keeping the model
                # scriptable for LAMMPS and compiled inference.
                for j, (field_feat, field_linear) in enumerate(
                    zip(self.field_feats, self.field_linear)
                ):
                    if i == j:
                        delta_node_feats = field_feat(
                            node_feats,
...
def is_macefield_model(model: torch.nn.Module) -> bool:
    """Return whether ``model`` supports the MACE-Field forward contract.

    The attribute checks are intentional: TorchScript, ``torch.compile`` and
    the CuEq/OEq conversion wrappers can obscure the original Python class,
    while the field modules remain part of the converted model. Common
    container wrappers (DDP, calculator wrappers, and TorchSim) are followed
    as well so callers do not need their own class-name checks.
    """
    candidates = [model]
    seen = set()
    for _ in range(8):
        ...
        if isinstance(candidate, MACEField):
            return True
        if hasattr(candidate, "field_feats") or hasattr(candidate, "field_linear"):
            return True
        for attr in ("_orig_mod", "module", "model"):
            wrapped = getattr(candidate, attr, None)
            if isinstance(wrapped, torch.nn.Module) and id(wrapped) not in seen:
